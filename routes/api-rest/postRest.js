...
exports.list = (request, response) => {
	keystone.list('Post').model.find((errors, list) => {
...
exports.create = (requests, response) => {
	let newProduct = new keystone.list('Post').model();
...
exports.delete = (request, response) => {
	...
	keystone.list('Post').model.findById(request.params.id).exec((errors, item) => {
		if (errors) return response.json({ errors: errors });
		if (!item) return response.json({ error: 'Not found' });
